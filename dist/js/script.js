window.addEventListener('DOMContentLoaded', () => {
	...
	$(document).ready(function() {
		...
		$(window).scroll(function() {
			...
			if ($(this).scrollTop() + height > $('.footer__info').offset().top) {
				const value = 15 + $(this).scrollTop() + height - $('.footer__info').offset().top + 'px';
				$('.pageup').css('bottom', value);
			}
			else 
			{
				$('.pageup').css('bottom', '15px');
			}
		});
